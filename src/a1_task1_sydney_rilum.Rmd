...
ggplot(data = yellow_frog_count, aes(x = year, y = total_count)) +
  geom_col(aes(fill = amphibian_life_stage)) +
  facet_wrap(~ amphibian_life_stage) +
  labs(x = "Year",
       y = "Total Count",
       title = "Total Mountain Yellow-Legged Frog Count by Life Stage") +
  theme_light() +
...
#frogs$lake_id <- as.character(as.numeric(frogs$lake_id)) 


#ggplot(data = yellow_frog_count_lakes, aes(y = lake_id, x = count)) +
  geom_col() 
